...
import { fetchLeaveTypes } from "./leaveTypeSlice";
import TableWrapper from "@/components/ui/TableWrapper";

type Inputs = z.infer<typeof leaveTypeZodSchema>;
...
const LeaveType = () => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    ...
    try {
      const res = await axios.post(`${BASE_URL}/leavetype`, data);

      if (res.status === 201) {
        toast.success(res.data.message || "Added Succesfully");
        reset();
